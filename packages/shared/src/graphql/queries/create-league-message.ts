...
import type { CreateLeagueMessageVars, CreateMessageResult } from "./types";
...
const MUTATION = `
  mutation create_message(
    ...
    create_message(
      parent_id: $parent_id, client_id: $client_id, parent_type: "league",
...
const MUTATION_WITH_ATTACHMENT = `
  mutation create_message(
    $parent_id: String!, $client_id: String!, $text: String,
    $attachment_type: String,
...
export async function createLeagueMessage(
  vars: CreateLeagueMessageVars,
): Promise<CreateMessageResult> {
  const hasAttachmentData = vars.k_attachment_data && vars.v_attachment_data;
  return gqlRequest<CreateMessageResult>(
    hasAttachmentData ? MUTATION_WITH_ATTACHMENT : MUTATION,
    {
      ...
      ...(hasAttachmentData
        ? { k_attachment_data: vars.k_attachment_data, v_attachment_data: vars.v_attachment_data }
        : {}),
    },
