    ...
    <div className="flex flex-col flex-1 items-center w-full gap-4">
      ...
      {tab === "ranks" && (
        <div className="w-full max-w-4xl space-y-0">
          {/* Value type + ADP config row */}
          <div className="flex items-center gap-3 flex-wrap rounded-t-xl border border-gray-700/80 bg-gradient-to-b from-gray-800 to-gray-800/80 px-4 py-2.5">
            {/* Value type selector */}
          ...
          {filter.valueType === "ktc" && (
            <div className="flex items-center gap-2 border-x border-gray-700/80 bg-gray-800/50 px-4 py-2">
          ...
          {/* Per-category Top N */}
          <div className="flex items-center gap-2 flex-wrap rounded-b-xl border border-t-0 border-gray-700/80 bg-gray-800/30 px-4 py-2">
            <span className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold shrink-0">Top N</span>
            {(["ALL", "PLAYERS", "QB", "RB", "WR", "TE", "PICKS"] as PositionFilter[]).map((pos) => (
              <div key={pos} className="flex items-center gap-1">
                <span className="text-[10px] text-gray-500">{RANK_LABELS[pos] ?? pos}</span>
                <input
                  type="number"
                  min={0}
                  placeholder="All"
                  ...
                  className="w-10 rounded-md bg-gray-900/60 border border-gray-700/60 px-1 py-0.5 text-[11px] text-gray-300 text-center placeholder:text-gray-600 focus:border-blue-500/50 focus:outline-none transition"
                />
              </div>
            ))}
          </div>
